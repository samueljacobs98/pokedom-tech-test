...
const getTypes = () => {
    const typesString = pokemonArray.map(pokemon => pokemon.types.join(" ")).join(" ")
...
const filterByType = () => {
    ...
    const filteredPokemonArray = pokemonArray.filter(pokemon => pokemon.types.includes(`${type}`))
    display(filteredPokemonArray)
}

const filterByName = () => {
    const name = filterNameSelected.value.toLowerCase()
    const filteredPokemonArray = pokemonArray.filter(pokemon => pokemon.name === `${name}`)
    display(filteredPokemonArray)
}
    ...
    fileteredPokemonArray.forEach((pokemon) => {
        ...
        `<div class="card">
            ...
            <div class="card__content">
                ...
                <p class="card__text">${name} (#${pokemon.id}) is a ${pokemon.types.join(" & ")} type pokemon.</p>
            </div>
        </div>`
        count++;
    })
